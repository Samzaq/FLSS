...
import torch
import numpy as np
# The import for Variable is no longer needed in modern PyTorch, 
# but I'll leave the logic as close to your original as possible.
from torch.autograd import Variable 
...
val_set    = torchvision.datasets.CIFAR10(root=args.data_dir, train=True , download=True, transform=transform_test)
test_set   = torchvision.datasets.CIFAR10(root=args.data_dir, train=False, download=True, transform=transform_test)

# Split training into train and balanced class 1000 images validation
train_size = 49000
valid_size = 1000
test_size  = 10000
...
#Here are the parameter names for the MLP(M)
MLP_param_name =  ["fc1_encoder.weight","fc1_encoder.bias","fc1_bn1_encoder.weight","fc1_bn1_encoder.bias","fc1_bn1_encoder.running_mean","fc1_bn1_encoder.running_var","fc2_encoder.weight"
        ,"fc2_encoder.bias","fc2_bn2_encoder.running_mean","fc2_bn2_encoder.running_var","fc2_bn2_encoder.weight","fc2_bn2_encoder.bias","fc3_encoder.weight","fc3_encoder.bias","fc4_encoder.weight","fc4_encoder.bias",
        "fc3_bn3_encoder.running_mean","fc3_bn3_encoder.running_var","fc3_bn3_encoder.weight","fc3_bn3_encoder.bias"]
####################################
#Training:
...
def train(args, model, device, train_loader, optimizer, epoch,scheduler,awp_adversary):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
    	...
    	#Calculate the KLD(sampled Adv||Adv) in the softmax space.
    	...
    	#Now calculate the loss KLD(sampled Adv||Adv) in softmax space.
        ...
        loss.backward()
        count=0
        #making the gradients zero for the classifier_encoder parameters
        for name, param in model.named_parameters():
            for strg in MLP_param_name:
                if str(strg) == str(name):
                    param.grad = None
                    count+=1
        #print(count)
        ...
        #Calculate the KLD(sampled_clean||clean(no samle)) in the softmax space.
        #Get the logits when no sampling is done
        #no_sample=True means it will not sample epsilon in latent space but rather take is as an input which is epso.
        ...
        pred_soft_no_sample = nn.Softmax(dim=1)(pred_no_sample)
        #Now calculate the loss KLD(sampled_clean||clean(no samle)) in softmax space.
...
######## Evaluations of models on Val Set Begins ###########
